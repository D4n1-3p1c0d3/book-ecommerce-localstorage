...
// Aggiungiamo questa funzione per caricare il carrello dal localStorage
function loadCartFromStorage() {
    ...
    if (savedCart) {
        ...
        cartTotal.innerText = savedTotal || '0.00';
        
        // Aggiorna il badge del carrello
        const itemCount = (savedCart.match(/class="d-flex justify-content-between/g) || []).length;
        document.querySelector('.badge').textContent = itemCount;
        
        // Ripristina i bordi rossi dei libri nel carrello
        // const cartAsins = Array.from(cartItems.querySelectorAll('button'))
        //     .map(btn => btn.getAttribute('onclick'))
        //     .filter(onclick => onclick)
        //     .map(onclick => onclick.match(/'([^']+)'/)[1]);
            
        cartAsins.forEach(asin => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadCartFromStorage(); // Aggiungiamo questa riga
});
...
const fetchBooks = () => {
            ...
            return ` <div class='col col-3'> <div class="card mb-4 shadow-sm" id='book_${book.asin}'>
                  ...
                  <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                      ...
                      <a href="dettagli.html?asin=${book.asin}" class="btn btn-info text-white">
  ...
  const addToCart = (title, price, asin) => {
    ...
    localStorage.setItem('cartTotal', cartTotal.innerText);
